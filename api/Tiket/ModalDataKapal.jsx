...
export default function ModalDataKapal({
  isVisible,
  onClose,
  onKapalSelected,
}) {
  ...
  const fetchDataKapal = async (pageNumber = 1, searchQuery = search) => {
    ...
    try {
      ...
      setDataKapal(
        pageNumber === 1 ? json.data : [...dataKapal, ...json.data],
      );
    } catch (error) {
  ...
  const pilihDataKapal = item => {
    onKapalSelected(item.kdkapal, item.namakapal, item.harga);
  };
  // Render Item untuk FlatList
  const renderItemKapal = ({item}) => (
    <TouchableOpacity
      onPress={() => pilihDataKapal(item)}
      style={{padding: 10, borderBottomWidth: 1, borderBottomColor: '#ccc'}}>
      <Text>{item.namakapal}</Text>
      <Text>{item.kdkapal}</Text>
